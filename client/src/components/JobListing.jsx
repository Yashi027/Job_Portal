...
import { assets, JobCategories, JobLocations, jobsApplied } from '../assets/assets.js'
    ...
    <div className='container 2xl:px-20 mx-auto flex flex-col lg:flex-row max-lg:space-y-8 py-8'>
      <div className='w-full lg:w-1/4 bg-white px-4'>
       ...
       <div className={showFilter? "" : "max-lg:hidden"}>
        ...
        <ul className='space-y-4 text-gray-600'>{JobCategories.map((category,index) => (
       ...
       <div className={showFilter? "" : "max-lg:hidden"}>
        ...
        <ul className='space-y-4 text-gray-600'>{JobLocations.map((location,index) => (
            <li className='flex gap-3 items-center' key={index}>
                <input type="checkbox" name="" id="" />
                {location}
            </li>
            ...
            jobs.length > 0 && (
                <div className='flex items-center justify-center space-x-2 mt-10'>
                    ...
                    {Array.from({length:Math.ceil(jobs.length/6)}).map((_,index) => (
                        <a href="#job-list">
                            <button onClick={() => setCurrentPage(index+1)} className={`flex items-center justify-center h-10 w-10 border border-gray-300 rounded ${currentPage==index+1 ? 'bg-blue-100 text-blue-500': 'text-gray-500'}`}>{index+1}</button>
                        </a>
                    ) )}
                    <a href="#job-list">
