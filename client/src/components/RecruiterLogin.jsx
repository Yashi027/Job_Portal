  ...
  return (
    <div className='absolute top-0 left-0 right-0 bottom-0 z-10 backdrop-blur-sm bg-black/30 flex justify-center items-center'>
      <form onSubmit={onSubmitHandler} className='relative bg-white p-10 rounded-xl text-slate-500'>
        <h1 className='text-center font-medium text-2xl text-neutral-800'>Recruiter {state}</h1>
        <p className='text-sm'>Welcome Back! Please SignIn to Continue</p>
        {
            ...
            <div className='flex items-center gap-4 my-10'>
                <label htmlFor="image">
                    <img className='w-14 h-14 rounded-full' src={image ? URL.createObjectURL(image) :assets.upload_area} alt="" />
        ...
        {
            state === "Login" 
            ? <p className='mt-5 text-center'>Don't have an account? <span className='text-blue-600 cursor-pointer' onClick={() => setState("SignUp")}>SignUp</span></p>
